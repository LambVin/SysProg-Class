...
int get_student(int fd, int id, student_t *s)
{
    ...
    return (s->id == 0) ? SRCH_NOT_FOUND : NO_ERROR;}
...
int add_student(int fd, int id, char *fname, char *lname, int gpa)
{
    ...
    s.gpa = gpa;
    off_t offset = id * STUDENT_RECORD_SIZE;
    if (lseek(fd, offset, SEEK_SET) < 0 || write(fd, &s, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
...
int del_student(int fd, int id)
{
    ...
    off_t offset = id * STUDENT_RECORD_SIZE;
    if (lseek(fd, offset, SEEK_SET) < 0 || write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int count_db_records(int fd)
{
    ...
    int count = 0;
    off_t offset = 0;
    while (pread(fd, &s, STUDENT_RECORD_SIZE, offset) == STUDENT_RECORD_SIZE) {
        if (s.id != 0) count++;
...
{
    // TODO
    int new_fd = open(TMP_DB_FILE, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
    if (new_fd < 0) {
