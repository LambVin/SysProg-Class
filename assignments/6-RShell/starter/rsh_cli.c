 ...
 */
 int exec_remote_cmd_loop(char *address, int port) {
    ...
    while (1) {
        ...
        int eof_found = 0;
        while (!eof_found) {
            ssize_t received = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0);
            if (received <= 0) break;

            if (rsp_buff[received - 1] == RDSH_EOF_CHAR) {
                eof_found = 1;
                received--; 
            }

            if (received > 0) {
                printf("%.*s", (int)received, rsp_buff);
            }
        }

        if (strcmp(cmd_buff, "stop-server") == 0) break;
    }
